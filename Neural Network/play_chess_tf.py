...
The AI uses your trained TFLite INT8 quantized model.

Install:
    pip install pygame python-chess tflite-runtime
    (or: pip install pygame python-chess tensorflow)
...
import chess
import numpy as np
# TFLite runtime — try full TF first (more stable), fall back to tflite_runtime
try:
    ...
    try:
        ...
        raise ImportError("Install tensorflow: pip install tensorflow==2.15.0")
...
MODEL_PATH  = os.path.join(SCRIPT_DIR, "chess_model.tflite")
...
# ─────────────────────────────────────────────────────────────
# MODEL — TFLite (no PyTorch needed)
# ─────────────────────────────────────────────────────────────
# Model is loaded by ChessAI below using tflite_runtime
...
def encode_board(board: chess.Board) -> np.ndarray:
    """Returns (18, 8, 8) float32 in standard NCHW order."""
...
def encode_board_nhwc(board: chess.Board) -> np.ndarray:
    """Returns (8, 8, 18) float32 in NHWC order as required by TFLite model."""
    return encode_board(board).transpose(1, 2, 0)   # (18,8,8) → (8,8,18)
...
class ChessAI:
    def __init__(self, model_path: str):
        ...
        mode = "INT8 fully quantized" if self.inp_dtype == np.int8 else "float32 hybrid quantized"
        print(f"  Input  : shape={self.inp['shape']}  dtype={self.inp_dtype.__name__}  scale={self.inp_scale:.6f}")
        print(f"  Output : shape={self.out['shape']}  dtype={self.out['dtype'].__name__}  scale={self.out_scale:.6f}")
        print(f"  Mode   : {mode}")
        print(f"  Model loaded from: {model_path}")

    def _run(self, board: chess.Board):
        # Encode board → NHWC float32 (1, 8, 8, 18)
        enc = encode_board_nhwc(board)[np.newaxis].astype(np.float32)

        inp_dtype = self.inp["dtype"]

        if inp_dtype == np.int8 or inp_dtype == np.uint8:
            # Fully quantized model — scale input to INT8
            if self.inp_scale != 0:
                enc_in = (enc / self.inp_scale + self.inp_zero).clip(-128, 127).astype(inp_dtype)
            else:
                # scale=0 with int8 dtype: values are already 0/1, cast directly
                enc_in = enc.astype(inp_dtype)
        else:
            # Float32/float16 input — pass directly (hybrid quantized model)
            enc_in = enc.astype(inp_dtype)

        self.interp.set_tensor(self.inp["index"], enc_in)
        self.interp.invoke()
        raw = self.interp.get_tensor(self.out["index"])[0]   # (4096,)

        # Dequantize output if INT8, otherwise use directly
        out_dtype = self.out["dtype"]
        if out_dtype == np.int8 or out_dtype == np.uint8:
            logits = (raw.astype(np.float32) - self.out_zero) * self.out_scale
        else:
